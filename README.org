...
- { and } :: Copy to the left and copy to the right respectively. Numeric argument defines how many cells to the left/right to copy to.
- ( and ) :: Scan the memory (to the left and right respectively) for zero cell and stop there. Numeric argument defines how many cells to skip though.
